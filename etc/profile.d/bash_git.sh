...
## You can change the separator between the branch name and the above state symbols by setting GIT_PS1_STATESEPARATOR. The default separator is SP (a space).
...
## When the repository has a sparse-checkout, a notification of the form "|SPARSE" will be included in the prompt. This can be shortened to a single '?' character by setting GIT_PS1_COMPRESSSPARSESTATE, or omitted by setting GIT_PS1_OMITSPARSESTATE.
...
## If you would like __git_ps1 to do nothing in the case when the current directory is set up to be ignored by git, then set GIT_PS1_HIDE_IF_PWD_IGNORED to a nonempty value. Override this on the repository level by setting bash.hideIfPwdIgnored to "false".
